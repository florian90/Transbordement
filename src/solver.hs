...
improveSolution pb i
    ...
    | otherwise = if isJust path
        then improveSolution newProblem (i-1)
        else pb
        where
...
coutSolution Problem {pb_nodes=nodes, pb_edges=edges, pb_solution=sol} = let
    coutElem (idEdge, nbrUtilisation)
        ...
        in foldl (\acc x -> acc + coutElem x ) 0 $ Map.assocs sol
...
-}
newBestSolution :: Problem -> Problem
newBestSolution pb = pb {pb_bestSolution=pb_solution pb, pb_bestSolutionCost=coutSolution pb}
